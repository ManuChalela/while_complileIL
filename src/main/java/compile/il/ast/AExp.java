...
public abstract class AExp {
    ...
    public static AExp generate(Random random, int min, int max) {
        ...
        switch (i) {
            //Terminals
            case 0:
                return Numeral.generate(random, min - 1, max - 1);
            case 1:
                return Variable.generate(random, min - 1, max - 1);
            //Non terminals
            case 2:
                return Multiplication.generate(random, min - 1, max - 1);
            case 3:
                return Addition.generate(random, min - 1, max - 1);
            case 4:
                return Subtraction.generate(random, min - 1, max - 1);
            default:
