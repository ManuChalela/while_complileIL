...
public abstract class BExp {
    ...
    public static BExp generate(Random random, int min, int max) {
        ...
        switch (i) {
            //Terminals
            case 0:
                return TruthValue.generate(random, min - 1, max - 1);
            //Non terminals
            case 1:
                return CompareEqual.generate(random, min - 1, max - 1);
            case 2:
                return CompareLessOrEqual.generate(random, min - 1, max - 1);
            case 3:
                return Negation.generate(random, min - 1, max - 1);
            case 4:
                return Conjunction.generate(random, min - 1, max - 1);
            default:
